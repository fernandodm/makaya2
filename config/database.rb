...
case Padrino.env
  ...
  when :staging 	  then DataMapper.setup(:default, ENV['HEROKU_POSTGRESQL_PURPLE_URL'])
  when :production  then DataMapper.setup(:default, ENV['HEROKU_POSTGRESQL_PURPLE_URL'])
end
